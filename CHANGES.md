...
- `SteamClient`/`CMClient` now default to the WebSocket CM transport (`protocol=ETransport.WebSocket`) instead of TCP. Pass `protocol=ETransport.TCP` explicitly to keep the previous behavior. `CMClient.PROTOCOL_TCP`/`PROTOCOL_UDP`/`PROTOCOL_WEBSOCKET` are removed in favor of `steam.enums.ETransport`
